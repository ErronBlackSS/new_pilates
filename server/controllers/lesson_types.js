...
async function Create (req, res) {
    ...
    const newLessonType = await pool.query('INSERT INTO lesson_types (name, description) VALUES ($1, $2)', [name, description])
...
async function Delete (req, res) {
    const { id } = req.body
    const lessonType = await pool.query('DELETE FROM lesson_types WHERE id = $1', [id])
    res.json(lessonType.rows[0])
}
